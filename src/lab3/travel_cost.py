...
def get_route_cost(route_coordinates, game_map):
    ...
    path = []
    dx = route_coordinates[1][0] - route_coordinates[0][0]
    dy = route_coordinates[1][1] - route_coordinates[0][1]
    
    total = max(abs(dy+1), abs(dx+1))
    
    #no change in x coordinates
    if dx == 0:
        
      path.append((np.full(total, route_coordinates[0][0]),
            np.round(np.linspace(route_coordinates[0][1], route_coordinates[1][1], total)).astype(np.int32)))
      
    #no change in y coordinates
    elif dy == 0:      
      path.append((np.round(np.linspace(route_coordinates[0][0], route_coordinates[1][0], total)).astype(np.int32),
                    np.full(total, route_coordinates[0][1])))
